...
if [[ -z "$EXEC_LINE" ]]; then
  echo "[에러] run_client.sh 형식을 인식하지 못했습니다."
...
list_current() {
  ...
  echo "HTTP   : $(echo "$EXEC_LINE" | grep -oE ' --http [^ ]+' | sed 's/^ --http //' || true)"
  echo "TCP    :"
...
for kv in "${TARR[@]}"; do
  ...
  [[ -z "$kv" ]] && continue
  if [[ "$kv" != *"="* || "$kv" != *":"* ]]; then
    echo "[경고] 잘못된 TCP: $kv  (예: web=127.0.0.1:8080)"
    continue
  fi
  name="${kv%%=*}"
  NEW_EXEC="$(remove_flag_by_name "$NEW_EXEC" "tcp" "$name")"
...
for kv in "${UARR[@]}"; do
  ...
  [[ -z "$kv" ]] && continue
  if [[ "$kv" != *"="* || "$kv" != *":"* ]]; then
    echo "[경고] 잘못된 UDP: $kv  (예: dns=127.0.0.1:53)"
    continue
  fi
  name="${kv%%=*}"
  NEW_EXEC="$(remove_flag_by_name "$NEW_EXEC" "udp" "$name")"
...
# 최신값 다시 파싱
[OWS_URL="$(echo "$EXEC_LINE" | sed -E 's#.*client\.py" "([^"]+)".*#\1#')"
SUBDOMAIN="$(echo "$EXEC_LINE" | sed -E 's#.*client\.py" "[^"]+" "([^"]+)".*#\1#')"
TOKEN="$(echo "$EXEC_LINE" | sed -E 's#.*client\.py" "[^"]+" "[^"]+" "([^"]+)".*#\1#')"
